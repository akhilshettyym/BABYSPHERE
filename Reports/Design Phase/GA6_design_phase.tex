...
\begin{itemize}
  ...
  \item React Native will be used for mobile app development, allowing cross-platform functionality for both Android and iOS.
  \item Firebase will serve as the backend for real-time data storage and user authentication, providing scalability and ease of integration.
  \item OpenCV will be utilized for computer vision tasks, specifically for detecting unsafe baby sleeping positions, and MQTT will be used as the messaging protocol to facilitate communication between the Raspberry Pi and the cloud.
\end{itemize}
...
\chapter{System Design}
This chapter describes the system design for the Cloud-Based Smart Monitoring System for Baby Health and Safety, detailing the architectural layout, functional design, control flow, access layer, and user interface flow. Each design diagram is explained with respect to the overall system.
\section{Abstract Design}
...
\begin{itemize}
  \item \textbf{IoT Sensors:} Sensors collect data on baby health parameters such as body temperature, room temperature, heart rate, SpO2, and movement.
  \item \textbf{Raspberry Pi:} Processes the data collected by the sensors and communicates with the cloud.
  \item \textbf{Cloud Infrastructure:} Firebase or a similar service stores the data and handles user authentication and real-time updates.
  \item \textbf{Mobile Application:} Provides the user interface for parents to monitor the baby’s health and receive alerts.
\end{itemize}
\begin{figure}[hbtp]
  \centering
...
\end{figure}
\subsection{Use Case Diagram}
The use case diagram shown in Figure \ref{fig:usecase} illustrates the interactions between the users (parents) and the system. Key use cases include monitoring health metrics, viewing live video feeds, receiving alerts for abnormal conditions, and managing user settings.
\begin{itemize}
  \item \textbf{Actors:} The primary actor is the parent who interacts with the system. Sensing system, mobile application and the overall system are the secondary actors.
  \item \textbf{Use Cases:} Include viewing baby health data, receiving alerts for unsafe conditions, adjusting settings, and exporting historical data.
\end{itemize}
\begin{figure}[hbtp]
  \centering
...
\section{Functional Design}
\subsection{Sequence diagram}
The sequence diagram shows the interactions between the system components over time for a particular functionality, such as generating an alert when an unsafe baby sleeping position is detected.
\begin{itemize}
  \item \textbf{Sensing system:} Continuously collect data and send it to the Raspberry Pi. Raspberry Pi processes the data and sends it to the cloud for storage and further processing.
  \item \textbf{Database:} Stores data and sends updates to the mobile application.
  \item \textbf{Mobile Application:} Notifies the parent if a dangerous condition is detected, such as the baby rolling onto their stomach.
\end{itemize}
\begin{figure}[hbtp]
  \centering
