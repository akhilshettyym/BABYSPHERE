...
The system's ability to detect unsafe sleeping postures, such as tummy and side sleeping, was tested using a variety of scenarios. 
\begin{itemize}
    ...
    \item Improvements were observed after optimizing the Mediapipe-based algorithms and adjusting the thresholds for posture recognition.
\end{itemize}
...
\section{Conclusion}
In conclusion, this project is reliable, accurate, and capable of meeting its intended objectives. The system offers a comprehensive solution to monitor key health metrics such as body temperature, heart rate, room temperature, humidity, and posture. By integrating real-time notifications and alerts, the system provides parents with peace of mind, ensuring that any abnormalities are promptly addressed. The innovative use of computer vision algorithms to monitor baby posture and prevent conditions like sudden infant death syndrome (SIDS) adds an extra layer of safety. 

The accuracy of the system was evaluated through various tests. The posture detection algorithm demonstrated high accuracy in identifying unsafe sleeping positions, such as tummy and side sleeping, with a high accuracy.% compared to manually labeled data. The system also monitored health parameters with an error margin of [XX]%, ensuring that body temperature, heart rate, SpO2 levels, and room conditions were accurately tracked. The experimental results confirmed the system's reliability and accuracy across different environmental conditions.

With the successful implementation and testing, the system not only improves infant safety but also reduces the need for constant parental supervision. The cloud-based design facilitates efficient remote monitoring, offering a resource-saving and energy-efficient approach while enhancing overall child care.
...
\begin{itemize}
    \item Detect unsafe sleeping postures, such as tummy and side sleeping, with a high accuracy.
    \item Monitor environmental parameters, including room temperature, humidity, heart rate, SpO2 saturation levels of the baby, and baby body temperature, ensuring a safe environment.
    % \item Identify crying patterns effectively to notify caregivers of potential distress.
    \item Deliver real-time alerts seamlessly to a React Native mobile application, enabling prompt actions by caregivers.
\end{itemize}

Overall, the project achieved its objectives, proving to be a reliable and efficient solution for infant monitoring. The combination of hardware and software systems ensures a comprehensive approach to baby health and safety.
